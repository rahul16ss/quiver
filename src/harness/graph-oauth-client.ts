...
export interface GraphTransport {
  ...
  ): Promise<{ status: number; body: GraphItemMetadata }>;
  /** Exchange a refresh token for a new access token. */
  refresh(
    refreshToken: string,
    opts: { tenantId: string; clientId: string; scope?: string },
  ): Promise<{ accessToken: string; refreshToken?: string; expiresIn?: number }>;
}

/** Default fetch-backed transport (real Microsoft Graph / Entra endpoints). */
export function defaultGraphTransport(
  graphBase = "https://graph.microsoft.com/v1.0",
): GraphTransport {
  const refreshEndpoint = (tenantId: string) =>
  ...
  return {
    ...
    async refresh(refreshToken, { tenantId, clientId, scope }) {
      ...
      const res = await fetch(refreshEndpoint(tenantId), {
        method: "POST",
...
export class GraphOAuthClient implements GraphClient {
  ...
  async getMetadata(itemId: string): Promise<GraphItemMetadata> {
    ...
    const { items } = await this.withRetry(tok.accessToken, (at) =>
      this.transport.get<{
        ...
        size?: number;
      }>(`/drives/root/items/${itemId}`, { accessToken: at }),
    );
    // Normalize to the GraphItemMetadata shape the provider consumes.
    const raw = items;
    return {
      id: itemId,
      name: raw.name ?? itemId,
      mimeType: "application/octet-stream",
      eTag: raw.eTag,
      webUrl: raw.webUrl,
      size: raw.size ?? 0,
      lastModified: raw.lastModifiedDateTime ?? new Date().toISOString(),
    };
  }
  ...
  uploadSession(itemId: string, _data: Buffer): Promise<GraphItemMetadata> {
  ...
  async delta(sku?: string): Promise<{ items: GraphItemMetadata[]; nextToken?: string }> {
    ...
    if ("error" in tok) throw new Error(tok.error);
    const { items } = await this.withRetry(tok.accessToken, (at) =>
      this.transport.get<any>(`/drive/root/delta`, { accessToken: at }),
    );
