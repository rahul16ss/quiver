...
export class KeychainTokenStore implements TokenStore {
  ...
  async get(name: string): Promise<string | null> {
    return getCredential(this.key(name));
  }
  async set(name: string, value: string): Promise<boolean> {
    return setCredential(this.key(name), value);
  }
  async delete(name: string): Promise<void> {
    return deleteCredential(this.key(name));
  }
}
...
export class ConnectorTokenManager {
  ...
  async persist(tokens: ConnectorTokens): Promise<void> {
    ...
    if (tokens.expiresAt !== undefined)
      await this.store.set(this.k(EXPIRES), String(tokens.expiresAt));
  }
