...
import type { ModelMessage, SensitivityProfile } from "./interfaces.js";
...
export class ModalityRouter {
  ...
  /**
   * Optional measured-routing hook (src/harness/routing-eval.ts). When
   * Pareto evidence exists for (role, modality) it wins; when the hook
   * returns null/undefined the static order below applies unchanged. The
   * hook is never consulted for restricted-mnpi routing (local-only).
   */
  constructor(
    profiles: ModelProfile[],
    private opts: {
      preferMeasured?: (role: ModelRole, modality: MessageModality) => string | null | undefined;
    } = {},
  ) {
  ...
  route(
    ...
    // 1b. Measured routing: Pareto evidence for this (role, modality) wins
    // over the static order — but only for a profile the pack allows to
    // serve this role. No evidence → static order, unchanged.
    const measured = this.opts.preferMeasured?.(role, modality);
    if (measured && this.servesRole(measured, role)) return measured;
