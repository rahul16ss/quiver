...
export interface DriveTransport {
  get<T>(
    path: string,
    opts: { params: Record<string, string>; accessToken: string },
  ): Promise<{ status: number; body: T }>;
  post<T>(
    path: string,
    opts: { accessToken: string; body: Buffer; contentType: string },
  ): Promise<{ status: number; body: T }>;
  refresh(
    refreshToken: string,
...
export function defaultDriveTransport(
  ...
  return {
    async get(path, { params, accessToken }) {
      ...
      const res = await fetch(`${driveBase}${path}${qs ? "?" + qs : ""}`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      return { status: res.status, body: (await res.json()) as never };
    },
    async post(path, { accessToken, body, contentType }) {
      const res = await fetch(`${driveBase}${path}`, {
        method: "POST",
    ...
    async refresh(refreshToken, { clientId, scopes }) {
      const form = new URLSearchParams({
        ...
        scope: scopes?.join(" ") ?? "https://www.googleapis.com/auth/drive.file",
      });
...
export class DriveOAuthClient implements DriveClient {
  ...
  private async bearer(): Promise<{ accessToken: string } | { error: string }> {
    ...
    if (t.refreshToken) {
      const r = await this.transport.refresh(t.refreshToken, {
        clientId: this.opts.clientId,
        scopes: this.opts.scopes,
      });
      if (r.accessToken) {
        await this.tok.persist({
  ...
  async getMetadata(fileId: string): Promise<DriveItemMetadata> {
    ...
    const { items } = await this.withRetry(tok.accessToken, (at) =>
      this.transport.get<{
        ...
        params: { fields: "id,name,mimeType,webViewLink,size,modifiedTime,headRevisionId,driveId" },
        accessToken: at,
      }),
    );
  ...
  async listChanges(
    ...
    const { items } = await this.withRetry(tok.accessToken, (at) =>
      this.transport.get<{
        changes?: Array<{ file?: { id: string; name?: string; mimeType?: string } }>;
        nextPageToken?: string;
      }>(`/changes`, {
        params: {
          pageSize: "100",
          pageToken: pageToken ?? "",
          fields: "changes(file(id,name,mimeType)),nextPageToken",
        },
        accessToken: at,
      }),
    );
    return {
      changes: (items.changes ?? [])
        .map((c) => (c.file ? DriveOAuthClient.fromFile({ ...c.file, id: c.file.id }) : null))
        .filter((m): m is DriveItemMetadata => !!m),
  ...
  private async withRetry<T>(
    initial: string,
    call: (accessToken: string) => Promise<{ status: number; body: T }>,
  ): Promise<{ items: T }> {
    let token = initial;
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      const ref = await this.transport.refresh(t.refreshToken, {
        clientId: this.opts.clientId,
        scopes: this.opts.scopes,
      });
      ...
      await this.tok.persist({
        ...
        refreshToken: ref.refreshToken ?? t.refreshToken,
        expiresAt: ref.expiresIn ? Date.now() + ref.expiresIn * 1000 : undefined,
      });
