...
export type Schedule = { type: "interval"; everyMs: number } | { type: "cron"; expr: string }; // stored + surfaced honestly; evaluation is app-specific
...
export class DurableJobScheduler {
  ...
  complete(jobId: string, nextDueAt: number): void {
    this.db
      .prepare(
        "UPDATE durable_jobs SET lease_holder=NULL, lease_until=NULL, attempts=0, next_due_at=? WHERE job_id=?",
      )
      .run(nextDueAt, jobId);
  }

  /** Record a failed attempt; after maxAttempts, dead-letter the job. */
  fail(jobId: string, error: string, maxAttempts: number, _now: number = Date.now()): boolean {
    const row = this.db.prepare("SELECT attempts FROM durable_jobs WHERE job_id = ?").get(jobId) as
      Row | undefined;
    const attempts = (row?.attempts ?? 0) + 1;
    if (attempts >= maxAttempts) {
      this.db
        .prepare(
          "UPDATE durable_jobs SET attempts=?, last_error=?, dead_lettered=1, lease_holder=NULL, lease_until=NULL WHERE job_id=?",
        )
    ...
    this.db
      .prepare(
        "UPDATE durable_jobs SET attempts=?, last_error=?, lease_holder=NULL, lease_until=NULL WHERE job_id=?",
      )
      .run(attempts, error, jobId);
    return false;
  }
  ...
  deadLettered(): JobRecord[] {
    const rows = this.db
      .prepare("SELECT * FROM durable_jobs WHERE dead_lettered=1")
  ...
  recover(jobId: string, at: number = Date.now()): void {
    this.db
      .prepare(
        "UPDATE durable_jobs SET dead_lettered=0, attempts=0, last_error=NULL, next_due_at=? WHERE job_id=?",
      )
      .run(at, jobId);
  }
...
export class DurableIdempotencyLedger {
  constructor(
    private kv: import("./cursor-store.js").CursorKV,
    private prefix = "quiver:seen:",
  ) {}
