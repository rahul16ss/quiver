...
export const tool: Tool = {
  name: "search_docs",
  description: "Queries Context7 to retrieve real-time, accurate, and up-to-date documentation context for a software library (e.g., next.js, react, supabase).",
  parameters: z.object({
    libraryName: z.string().describe("The name of the library (e.g., 'next.js', 'react', 'supabase')."),
    query: z.string().describe("The documentation topic or API signature you want to lookup."),
  }),
  execute: async ({ libraryName, query }) => {
    ...
    if (apiKey) {
      ...
      console.log("\nℹ️  [Context7] No API key configured in .env. Querying public endpoint (rate limits apply).\n");
    }

    try {
      // Step 1: Search for libraryId
      const searchRes = await fetch(`https://context7.com/api/v2/libs/search?libraryName=${encodeURIComponent(libraryName)}`, {
        headers,
      });
      ...
      const contextRes = await fetch(`https://context7.com/api/v2/context?libraryId=${encodeURIComponent(libraryId)}&query=${encodeURIComponent(query)}&type=json`, {
        headers,
      });
      ...
      const formatted = snippets.map((snip: any, idx: number) => {
