...
export const tool: Tool = {
  ...
  parameters: z.object({
    ...
    filePath: z
      .string()
      .optional()
      .describe("Local path to the file to export (required for 'export')."),
    name: z
      .string()
      .optional()
      .describe("The document name in the DMS (defaults to the file's basename)."),
    fields: z
      .record(z.string(), z.string())
      .optional()
      .describe("Free-form metadata mapped to DMS fields (matter, client, author…)."),
  }),
  async execute(args) {
    loadDmsConfig();
    if (args.action === "list") {
    ...
    if (args.action === "status") {
      ...
      return e
        ? `Active DMS: ${e.id} (${e.isConfigured() ? "configured" : "not configured — " + e.configHint()})`
        : 'No active DMS adapter. Configure .quiver/dms.json (e.g. { "active": "sharepoint" }) and the adapter\'s env vars.';
    }
    // export
    ...
    const e = getActiveDmsExporter();
    if (!e)
      return 'No active DMS adapter configured. Set .quiver/dms.json { active: "sharepoint" } and the adapter\'s env vars (see dms_export status).';
