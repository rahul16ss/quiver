...
npm run export:website-demo # copy the buyer-facing pages into website-export/ (after the demo)
```
...
| `Project_Alder_Evidence.html` | Business-readable evidence report (screenshotted to `screenshots/`) |
| `Project_Alder_Review_Checklist.md` | Reviewer checklist (machine-readable): 3 flagged figures with what to verify, plus the standing checks |
| `Project_Alder_Review_Checklist.html` | Business-readable reviewer checklist: same groups as the Markdown, with the unresolved item made prominent |
| `Project_Alder_Run_Record.json` | Inputs with SHA-256 hashes, sources used/excluded, tool versions, `draft_for_review` status |
| `Project_Alder_Provenance.html` | Business-readable provenance page ("What informed this draft"): inputs, exclusions, the acceptance checks with pass states, workflow status |
...
- The export script's hygiene checks (no absolute local paths, no
  secret-looking strings, label present in every artifact, no developer
  vocabulary on the buyer-facing HTML pages).
...
website-export/            hygiene-checked copies for the public site (committed):
                           the three business-readable HTML pages + Evidence.json
```
