...
| 2 | `ModelClient` + `QuiverOpenRouterClient` + `LocalModelClient` + `ModelProfile` + `QuiverOpenRouterProvider` bridge + native-doc contract-test scaffolding | `02-model-client.test.ts` (21) + `14-provider-bridge.test.ts` (19) + opt-in `live/run.ts` | ✅ `getActiveProvider()` prefers the OpenRouter bridge when `OPENROUTER_API_KEY`+profile set | Remove `OpenAICompatibleProvider` cloud path (full removal gated on spec updates) | ✅ adapter + bridge + flip done; full legacy removal pending |
...
| 5 | `PromptCompiler` + modular capital-markets prompt packs | `01-interfaces.test.ts` + `13-customer-pack-fixture.test.ts` (29) | ✅ `assemblePrompt` pack-injection seam inserts customer-pack + domain-policy layers when a CustomerPack is bound (legacy sections preserved) | Remove scattered prompts after full delegation | ✅ compiler + seam + shipped pack done; full assembler delegation pending |
