...
It demonstrates how Quiver's security-first, inspectable design supports SOC2 compliance for
enterprise deployments. Data handling and model use are configured around the workflow's
sensitivity; the model endpoint is user-configured (no baked default) — treat it as remote
unless a local endpoint is set.
...
| CC6.2    | `src/secrets/keychain.ts`              | OS credential store integration (macOS Keychain, Windows Credential Manager). API keys never stored in plain text. |
| CC6.3    | `src/secrets/env_fallback.ts`          | `.env` fallback with restrictive permissions (0600). Auto-added to `.gitignore`. Excluded from model context.      |
...
| CC8.4    | `src/registry.ts`        | Static tool registry. Shipped tools load from `src/tools/` at startup; there is no runtime tool synthesis path. |

### CC9 — Risk Mitigation

| Criteria | Quiver Subsystem             | Implementation                                                                                                 |
| -------- | ---------------------------- | -------------------------------------------------------------------------------------------------------------- |
| CC9.1    | `src/memory/privacy.ts`      | Memory privacy labels: public, project, private, secret. Secret-labeled memories never sent to remote models.  |
| CC9.2    | `src/memory/review_queue.ts` | Memory review queue. Extracted facts enter pending state. User must accept, edit, or reject before activation. |
...
| Criteria | Quiver Subsystem             | Implementation                                                                        |
| -------- | ---------------------------- | ------------------------------------------------------------------------------------- |
| P1.1     | `src/memory/privacy.ts`      | Memory privacy filtering applied before prompt assembly for remote providers.         |
| P1.2     | `src/memory/review_queue.ts` | Extracted memories enter pending state. User controls what becomes permanent context. |
| P1.3     | `src/session_logger.ts`      | Log retention commands allow users to manage and purge accumulated data.              |
...
The audit chain is tamper-evident (SHA-256 hash-chained) and is verified by the
acceptance gate's audit-chain checks (`verifyAuditChain()` in `src/logger.ts` and
`src/session/checkpoint.ts`), not by a user-facing command.

### Automated gates

```bash
npm test            # The single checker-owned acceptance contract — asserts the SPEC
                     # AND that the agent loop actually wires the architecture in (WIRE-* checks)
npx tsc --noEmit    # Definition of Done: clean typecheck
```

The gate is the single checker-owned acceptance contract, read-only to the vendor;
there is no separate wiring suite — the `WIRE-*` integration checks are part of
`npm test`. `npm test` is the live verdict — re-run it for the current status; see
`tests/ACCEPTANCE_CONTRACT.md`.
